# Final-Project---Virtual-Reality
Final Project for Virtual Reality Programming


Description: 
This project will feature a concept of a chase type virtual reality game in which players would be have to use map available structures to deter the enemy programmed to chase and kill them.



# Game Overview


Game Name: TBD


How to play -->


Goal for Player: Survive for 2 minutes (STC)
Goal for Enemy: Kill/ stop the Player from survivng the set duration by any means necessary


# Gameplay:



Player Name: Pending (TBD)


Players can collect enhancements provided around the map to aid in survivng for longer durations

Enchancements:
Speed Boost - Increases players movement speed by 20% temporarily (5s)
Durability: If Player is hit they will not be slowed // If hit by an ability will have less of an effect
Weapon/Stun: Can be used to Stun the Enemy and temporarily stop its movements
Heal/Health Boost: Can be picked up if a player attempting to survive has been hit. Will give the player a heal over time effect. If player has not been it, it will provide a small health boost/




Enemy Name: Pending (TBD)

Weapon - Knife/Bat/Hand
Primary Ability - Some form of a damage and slow 
Secondary Ability - Used as more of a buff to itself ( Increase in speed, faster hit recovery, reduced stun time )




# Map - 


Features -

Kind of a Big Dome featuring obstacles, natural scenery and more

Traps - Obstacles the map features that will distract/deter both player and enemy. 


# Roadmap - Development Process/ Ideas

Start off building a foundation for the map adding necessary features and scenery.
Progress through building models for the Player and Enemy



By: Daishunn Farquharson
